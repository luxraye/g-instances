...
export default async function AdminInstancesPage() {
  ...
  return (
    <div className="max-w-4xl space-y-8">
      <div>
        <h1 className="text-2xl font-bold" style={{ color: "var(--brand)" }}>All Submissions</h1>
      ...
      <div>
        ...
        <div className="rounded-xl border bg-white overflow-hidden" style={{ borderColor: "var(--border)" }}>
              ...
              {instances.map((i, idx) => (
                <tr key={i.id} style={{ borderTop: idx === 0 ? undefined : "1px solid #f0f0f0" }}>
                  <td className="px-4 py-3">
                    ...
                    <p className="text-xs" style={{ color: "var(--text-secondary)" }}>{i.assignee.email}</p>
                  </td>
                  <td className="px-4 py-3">
                    <p className="text-sm" style={{ color: "#222733" }}>{i.template.name}</p>
                    <p className="text-xs font-mono" style={{ color: "var(--text-secondary)" }}>{i.template.code}</p>
                  </td>
          ...
          {instances.length === 0 && (
            <p className="px-4 py-6 text-center text-sm" style={{ color: "var(--text-secondary)" }}>No submissions yet.</p>
          )}
        </div>
      </div>
    </div>
