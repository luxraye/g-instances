...
const inputStyle = { borderColor: "var(--border)", background: "#ffffff" };
...
export function MatrixAdmin({ initialDimensions }: { initialDimensions: Dim[] }) {
  ...
  return (
    <div className="space-y-8 mt-2">
      <form onSubmit={addDimension} className="rounded-xl border bg-white p-5 space-y-4" style={{ borderColor: "var(--border)" }}>
        <h2 className="text-sm font-semibold" style={{ color: "var(--brand)" }}>Add classification dimension</h2>
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--text-primary)" }}>Dimension name</label>
            <input
          ...
          <div>
            <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--text-primary)" }}>Slug (URL-safe)</label>
            <input
              className={inputCls} style={inputStyle}
      ...
      {initialDimensions.length === 0 ? (
        <p className="text-sm" style={{ color: "var(--text-secondary)" }}>No dimensions yet. Add one above.</p>
      ) : (
  ...
  return (
    <div className="rounded-xl border bg-white p-5" style={{ borderColor: "var(--border)" }}>
      <div className="flex items-center gap-2 mb-3">
        <h3 className="text-sm font-semibold" style={{ color: "var(--brand)" }}>{dimension.name}</h3>
        <code className="text-xs rounded px-1.5 py-0.5 font-mono" style={{ background: "var(--brand-light)", color: "var(--brand-mid)" }}>
          {dimension.slug}
        </code>
      </div>
      ...
      {dimension.values.length > 0 ? (
        <div className="flex flex-wrap gap-2 mb-4">
          {dimension.values.map((v) => (
            ...
            >
              {v.label}
              <code className="ml-1.5 font-mono text-xs" style={{ color: "var(--text-secondary)" }}>{v.code}</code>
            </span>
          ))}
        </div>
      ) : (
