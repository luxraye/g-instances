...
export function ApiKeyClient({ initialKeys }: { initialKeys: KeyRow[] }) {
  ...
  return (
    <div className="space-y-8">
      <form onSubmit={createKey} className="rounded-xl border bg-white p-5 space-y-4" style={{ borderColor: "var(--border)" }}>
        <h2 className="text-sm font-semibold" style={{ color: "var(--brand)" }}>Generate new API key</h2>
        ...
        <div>
          <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--text-primary)" }}>Key label</label>
          <input
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: "var(--bg-subtle)", borderBottom: "1px solid var(--border)" }}>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--text-secondary)" }}>Label</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--text-secondary)" }}>Prefix</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--text-secondary)" }}>Created</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--text-secondary)" }}>Last used</th>
                </tr>
              </thead>
              <tbody>
                {initialKeys.map((k, i) => (
                  <tr key={k.id} style={{ borderTop: i === 0 ? undefined : "1px solid #f0f0f0" }}>
                    ...
                    <td className="px-4 py-3">
                      <code className="font-mono text-xs" style={{ color: "var(--text-secondary)" }}>{k.keyPrefix}...</code>
                    </td>
