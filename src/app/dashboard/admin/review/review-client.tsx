...
export function ReviewClient({ instances }: { instances: Inst[] }) {
  ...
  if (!instances.length) {
    return (
      <div className="mt-8 rounded-xl border bg-white p-10 text-center" style={{ borderColor: "var(--border)" }}>
        <p className="text-4xl mb-3">✅</p>
        <p className="font-medium" style={{ color: "var(--brand)" }}>Queue is clear</p>
        <p className="text-sm mt-1" style={{ color: "var(--text-secondary)" }}>No submissions are awaiting review.</p>
      </div>
    );
  }
          ...
          {instances.map((i) => (
            <li key={i.id}>
              <button
                ...
                style={{
                  background: selected === i.id ? "var(--brand-light)" : "#ffffff",
                  borderColor: selected === i.id ? "var(--brand-mid)" : "var(--border)",
                }}
              >
                <p className="text-xs font-semibold truncate" style={{ color: "var(--brand)" }}>{i.template.code}</p>
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-xl border bg-white p-5" style={{ borderColor: "var(--border)" }}>
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <h2 className="text-base font-bold" style={{ color: "var(--brand)" }}>{current.template.name}</h2>
                <p className="text-xs font-mono mt-0.5" style={{ color: "var(--text-secondary)" }}>{current.template.code}</p>
              </div>
              <StatusBadge status={current.status} />
            </div>
            <div className="mt-3 grid grid-cols-2 gap-3 text-xs">
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide mb-0.5" style={{ color: "var(--text-secondary)" }}>Organisation</p>
                <p style={{ color: "#222733" }}>{current.assignee.name}</p>
                <p style={{ color: "var(--text-secondary)" }}>{current.assignee.email}</p>
              </div>
              <div>
                <p className="text-xs font-semibold uppercase tracking-wide mb-0.5" style={{ color: "var(--text-secondary)" }}>Timeline</p>
          ...
          {/* Payload */}
          <div className="rounded-xl border bg-white p-5" style={{ borderColor: "var(--border)" }}>
            <h3 className="text-sm font-semibold mb-3" style={{ color: "var(--brand)" }}>Submission payload</h3>
            <pre
          ...
          {current.files.length > 0 && (
            <div className="rounded-xl border bg-white p-5" style={{ borderColor: "var(--border)" }}>
              <h3 className="text-sm font-semibold mb-3" style={{ color: "var(--brand)" }}>Attached files</h3>
          ...
          {/* Decision */}
          <div className="rounded-xl border bg-white p-5" style={{ borderColor: "var(--border)" }}>
            <h3 className="text-sm font-semibold mb-3" style={{ color: "var(--brand)" }}>Decision</h3>
            <textarea
