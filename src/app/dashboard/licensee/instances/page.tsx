...
export default async function LicenseeInstancesPage() {
  ...
  const statCards = [
    { label: "Total",     value: stats.total,     icon: <FileBox size={20} />,       color: "var(--brand-mid)" },
  ...
  return (
    <div className="max-w-5xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold" style={{ color: "var(--brand)" }}>My Submissions</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {statCards.map((s) => (
          <div key={s.label} className="rounded-xl bg-white p-4" style={{ border: "1px solid var(--border)" }}>
            <div
      ...
      {instances.length === 0 ? (
        <div className="rounded-xl bg-white p-12 text-center" style={{ border: "1px solid var(--border)" }}>
          <FileBox size={36} className="mx-auto mb-3" style={{ color: "var(--border)" }} />
          <p className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>No submissions yet</p>
        ...
        <div className="rounded-xl bg-white overflow-hidden" style={{ border: "1px solid var(--border)" }}>
          <div className="px-5 py-4" style={{ borderBottom: "1px solid var(--border)" }}>
            <h2 className="text-sm font-semibold" style={{ color: "var(--brand)" }}>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: "var(--bg-subtle)", borderBottom: "1px solid var(--border)" }}>
                  <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--text-secondary)" }}>Service</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--text-secondary)" }}>Code</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--text-secondary)" }}>Deadline</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--text-secondary)" }}>Status</th>
                  <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--text-secondary)" }}>Action</th>
                </tr>
              </thead>
              <tbody>
                {instances.map((inst, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-3.5">
                        <code className="text-xs font-mono font-semibold" style={{ color: "var(--brand-mid)" }}>
                          {inst.template.code}
                        </code>
                      </td>
                      <td className="px-5 py-3.5 text-xs" style={{ color: overdue ? "#dc2626" : "var(--text-secondary)" }}>
                      ...
                      <td className="px-5 py-3.5">
                        {canOpen ? (
                          <Link
                            ...
                            style={{ color: inst.status === "FLAGGED" ? "#c2410c" : "var(--brand-mid)" }}
                          >
