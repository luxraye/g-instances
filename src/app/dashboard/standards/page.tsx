...
export default async function StandardsPage() {
  ...
  return (
    <div className="max-w-5xl space-y-8">
      <div>
        <h1 className="text-2xl font-bold" style={{ color: "var(--brand)" }}>Standards Catalogue</h1>
        <p className="mt-1 text-sm" style={{ color: "var(--text-secondary)" }}>
           Standards (BOS) published by the Bureau. Purchase via the{" "}
          <a href="https://bobstandards.bw/shop/" target="_blank" rel="noopener" className="hover:underline" style={{ color: "var(--brand-mid)" }}>
            Your organisation Webstore
          </a>.
        </p>
      </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ background: "var(--bg-subtle)", borderBottom: "1px solid var(--border)" }}>
                    <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--text-secondary)" }}>Reference</th>
                    <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--text-secondary)" }}>Title</th>
                    ...
                    <th className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: "var(--text-secondary)" }}>Action</th>
                  </tr>
                </thead>
                <tbody>
                  {sectorStandards.map((std, i) => (
                    <tr key={std.ref} style={{ borderTop: i === 0 ? undefined : "1px solid #f0f0f0" }}>
                      <td className="px-4 py-3">
                        <p className="font-mono text-xs font-semibold" style={{ color: "var(--brand)" }}>{std.ref}</p>
                        {std.iso && (
                          <p className="text-xs" style={{ color: "var(--text-secondary)" }}>≡ {std.iso}</p>
                        )}
                      </td>
                      <td className="px-4 py-3">
      ...
      <p className="text-xs text-center pb-4" style={{ color: "#9ca3af" }}>
        Showing {STANDARDS.length} of 300+  Standards. For the complete catalogue,{" "}
        <a
          href="https://bobstandards.bw/wp-content/uploads/2024/06/Your organisation-Standards-Catalogue-June-2024.pdf"
          target="_blank"
          rel="noopener"
